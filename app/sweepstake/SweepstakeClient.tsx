...
function statusLabel(s: string): string {
  return ({ safe: 'Safe', warning: 'Warning', danger: 'Danger', bust: 'Bust' } as Record<string, string>)[s] ?? s
}

function GoalBar({ total, status, showMeta = true }: { total: number; status: string; showMeta?: boolean }) {
  const pct = goalPct(total)
  const colour = ({ safe: '#22873E', warning: '#C8860A', danger: '#C0392B', bust: '#7F1D1D' } as Record<string, string>)[status] ?? '#22873E'
  return (
    <div style={{ width: '100%' }}>
      {showMeta && (
      ...
      <div style={{ background: 'var(--ink3)', borderRadius: 2, overflow: 'hidden', height: 3 }}>
...
function StatusPill({ status }: { status: string }) {
  ...
  const c = styles[status] ?? { bg: 'transparent', text: '#666', border: 'transparent' }
  return (
    <span style={{
...
function LbCard({ entry }: { entry: SweepstakeEntry }) {
  const isBust = entry.status === 'bust'
  return (
    <div style={{ borderBottom: '1px solid var(--rule2)', padding: '12px 16px', opacity: isBust ? 0.35 : 1 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 9 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          ...
          <span style={{
            fontSize: 15, fontWeight: 800, letterSpacing: '-.02em', textTransform: 'uppercase',
            color: 'var(--text)', textDecoration: isBust ? 'line-through' : 'none',
          }}>
            {entry.name}
          </span>
...
export default function SweepstakeClient() {
  ...
  if (error && !data) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, gap: 12 }}>
        <p style={{ color: 'var(--text)', fontWeight: 700, fontSize: 16 }}>Something went wrong</p>
        <p style={{ color: 'var(--muted)', fontSize: 13, textAlign: 'center', maxWidth: 300 }}>{error}</p>
        <button
          onClick={fetchData}
          style={{ background: 'var(--ink2)', border: '1px solid var(--rule)', color: 'var(--text)', padding: '8px 16px', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}
        >
          Try again
        </button>
      </div>
  ...
  const bustedCount = data.entries.filter(e => e.status === 'bust').length
    ...
    <div style={{ maxWidth: 480, margin: '0 auto' }}>
      <style>{`
        @keyframes heartbeat {
          ...
          40%      { box-shadow: inset 0 0 0 1px rgba(192,57,43,.45), 0 0 28px rgba(192,57,43,.06); }
        }
      ...
      {/* ── PITCH HEADER ──────────────────────────────────────────────── */}
      <header style={{ background: 'var(--pitch)', position: 'relative', overflow: 'hidden' }}>
        {/* CSS pitch markings */}
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '20px 16px 16px' }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.55)', marginBottom: 6 }}>
            World Cup 2026 · Sweepstake Tracker
          </div>
          <div style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-.04em', lineHeight: 1.05, color: '#fff', marginBottom: 2 }}>
            STAY<br />UNDER 21
          </div>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginBottom: 14 }}>
            Go over and you're out. No exceptions.
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr 1px 1fr', background: 'rgba(0,0,0,0.25)', borderRadius: 6, overflow: 'hidden' }}>
            {[
              { val: alive,       color: '#6EE89A', label: 'Alive' },
              { val: null,        color: '',        label: '' },
              { val: data.totalGoalsScored, color: '#fff', label: 'Goals' },
              { val: null,        color: '',        label: '' },
              { val: bustedCount, color: '#FF8A80', label: 'Busted' },
            ].map((item, i) =>
              item.val === null
                ? <div key={i} style={{ background: 'rgba(255,255,255,0.1)', margin: '8px 0' }} />
                : (
                  <div key={i} style={{ padding: '10px 0', textAlign: 'center' }}>
                    <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: '-.04em', lineHeight: 1, color: item.color }}>{item.val}</div>
                    <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)', marginTop: 2 }}>{item.label}</div>
                  </div>
                )
            )}
          </div>
      ...
      {/* ── NEXT MATCH BAR ────────────────────────────────────────────── */}
      <div style={{ background: 'var(--ink2)', borderBottom: '1px solid var(--rule)', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {liveMatch ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginLeft: 4 }}>{liveMatch.homeTeam} vs {liveMatch.awayTeam}</span>
            </div>
          ...
          <span style={{ fontSize: 12, color: 'var(--muted)' }}>Updated {formatRelative(data.lastUpdated)}</span>
        )}
      </div>

      {/* ── TABS ──────────────────────────────────────────────────────── */}
      <nav style={{ background: 'var(--ink2)', borderBottom: '1px solid var(--rule)', display: 'flex', overflowX: 'auto', position: 'sticky', top: 0, zIndex: 20 }}>
        {(['overview', 'table', 'goals', 'teams', 'shame'] as const).map(id => (
          <button
            key={id}
            onClick={() => setTab(id)}
            style={{
          ...
          >
            {({ overview: 'Overview', table: 'Table', goals: 'Goals', teams: 'Teams', shame: 'Shame' } as Record<string, string>)[id]}
          </button>
        ))}
      </nav>
      ...
      {tab === 'overview' && (
        <>
          ...
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
            {headlines.length === 0
              ? <div style={{ padding: '14px 16px', color: 'var(--muted)', fontSize: 13 }}>Tournament hasn't started yet.</div>
              : headlines.map((h, i) => (
                <div key={i} style={{ padding: '11px 16px', borderBottom: i < headlines.length - 1 ? '1px solid var(--rule2)' : 'none', display: 'flex', gap: 10, alignItems: 'baseline' }}>
          ...
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--rule)' }}>
            {sweat ? (
              <div style={{ background: 'var(--red-bg)', border: '1px solid rgba(192,57,43,0.3)', borderRadius: 8, padding: 14, animation: 'heartbeat 2.8s ease-in-out infinite' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(192,57,43,0.6)', textTransform: 'uppercase', marginTop: 2 }}>{sweat.remaining} from bust</div>
                  </div>
                </div>
              ...
              <div style={{ color: 'var(--muted)', fontSize: 13 }}>Tournament hasn't started yet.</div>
            )}
          </div>
          ...
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
            {danger.length === 0
              ? <div style={{ padding: '14px 16px', color: 'var(--muted)', fontSize: 13 }}>Everyone comfortable. For now.</div>
              : danger.slice(0, 6).map((e, i) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: i < Math.min(danger.length, 6) - 1 ? '1px solid var(--rule2)' : 'none', background: e.status === 'danger' ? 'rgba(192,57,43,0.04)' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 900, color: 'var(--dim)', width: 16, textAlign: 'center' }}>{e.rank}</span>
            ...
            <div style={{ position: 'relative', paddingLeft: 20 }}>
              ...
              {[
                ...
                <div key={i} style={{ position: 'relative', paddingBottom: 16 }}>
                  <div style={{ position: 'absolute', left: -18, top: 5, width: 8, height: 8, borderRadius: '50%', background: 'dot' in item ? item.dot : '#666', border: '1.5px solid var(--ink)' }} />
                  ...
                  <div style={{ fontSize: 12, fontWeight: 500, color: 'dim' in item && item.dim ? 'var(--dim)' : 'var(--text2)', lineHeight: 1.4 }}>{item.text}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'table' && (
        <div style={{ borderBottom: '1px solid var(--rule)' }}>
          {active.map(e => <LbCard key={e.id} entry={e} />)}
          {bustList.length > 0 && (
            <>
              <SectionDivider label="Eliminated" />
              {bustList.map(e => <LbCard key={e.id} entry={e} />)}
            </>
          )}
        </div>
      )}
      ...
      {tab === 'goals' && (
        <>
          <SectionDivider label="Goal feed" />
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
                    ...
                    <div key={ev.id} style={{ borderBottom: '1px solid var(--rule2)', padding: '12px 16px', background: i === 0 ? 'rgba(200,134,10,0.03)' : 'transparent' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
                        <div style={{ fontSize: 13, fontWeight: 700 }}>
                          {ev.homeTeam} <span style={{ fontWeight: 900 }}>{ev.homeScore}–{ev.awayScore}</span> {ev.awayTeam}
                        </div>
                        <span style={{ fontSize: 10, color: 'var(--dim)', flexShrink: 0, marginLeft: 8 }}>{formatRelative(ev.matchTime)}</span>
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: affected.length ? 7 : 0 }}>
                        ⚽ {ev.scoringTeam} scored {ev.minute > 0 ? `(${ev.minute}')` : ''}
                        {ev.isExtraTime && <span style={{ marginLeft: 6, fontSize: 8, fontWeight: 800, background: 'rgba(200,134,10,0.15)', color: '#F5B942', padding: '1px 5px', borderRadius: 3 }}>AET</span>}
                      </div>
                      {affected.length > 0 && (
                        <>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {affected.map(e => (
                              <span key={e.id} style={{
                                ...
                                {e.name} +{ev.scoringTeam === e.teams.find(t => t.name === ev.scoringTeam)?.name ? (e.teams.find(t => t.name === ev.scoringTeam)?.goals ?? 1) : 1}
                              </span>
                            ))}
                          </div>
                        </>
          ...
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
            {data.teamSummaries.filter(t => t.goals > 0).slice(0, 8).map((t, i) => (
              <div key={t.name} style={{ display: 'flex', alignItems: 'center', padding: '9px 16px', borderBottom: '1px solid var(--rule2)', gap: 10 }}>
                <span style={{ fontSize: 9, fontWeight: 900, color: 'var(--dim)', width: 12, textAlign: 'center' }}>{i + 1}</span>
                ...
                {openTeam === t.name && (
                  <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--rule)' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {t.ownerNames.map((n, idx) => (
                        <span key={idx} style={{ fontSize: 10, fontWeight: 600, background: 'var(--ink3)', border: '1px solid var(--rule)', borderRadius: 3, padding: '2px 6px', color: 'var(--text2)' }}>{n}</span>
                      ))}
                    </div>
                  </div>
                )}
        ...
        <>
          ...
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
            {danger.length === 0
              ...
              : danger.slice(0, 4).map(e => <LbCard key={e.id} entry={e} />)}
          </div>

          <SectionDivider label="Hall of shame" />
          <div style={{ borderBottom: '1px solid var(--rule)' }}>
            {bustList.length === 0
              ? <div style={{ padding: 24, textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>Nobody out yet. Early doors.</div>
              : bustList.map(e => (
