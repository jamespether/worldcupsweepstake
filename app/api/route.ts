...
interface FDScore {
  ...
  // Present once a match reaches extra time / penalties. `regularTime` is the
  // 90-minute score; `extraTime` and `penalties` are NOT cumulative — they
  // hold only the goals scored within that specific period.
...
//
// football-data.org's `score.fullTime` is NOT safe to use directly for
// matches decided by penalties: fullTime = regularTime + extraTime +
// penalties, so it silently folds shootout goals into the total.
//
// `score.regularTime` (90-min score) and `score.extraTime` (goals scored
// ONLY within extra time, not cumulative) give us exactly what we want:
// regulation + extra time, with shootout goals excluded entirely.
...
function countGoals(match: FDMatch): { home: number; away: number } {
  ...
  // Fallback for the rare case regularTime is missing: strip penalty goals
  // out of fullTime manually rather than trusting fullTime as-is.
  return {
    home: (match.score.fullTime.home ?? 0) - (match.score.penalties?.home ?? 0),
    away: (match.score.fullTime.away ?? 0) - (match.score.penalties?.away ?? 0),
  }
}
